...
function redirectToFrontend(status: 'connected' | 'error', message?: string) {
  ...
  url.searchParams.set('google_analytics', status);
...
export async function GET(request: NextRequest) {
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    return redirectToFrontend('error', 'google_analytics_oauth_not_configured');
  }
  ...
  try {
    ...
    console.error('[Google Analytics Callback] Error:', callbackError);
