...
import { createGoogleBusinessState } from '@/lib/googleBusiness';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const params = new URLSearchParams({
      ...
      state: createGoogleBusinessState(userId),
    });
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to start Google Business connection' },
      { status: 500 }
    );
  }
}