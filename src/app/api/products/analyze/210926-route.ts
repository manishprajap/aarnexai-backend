...
const DEFAULT_ASPECT_RATIO: '1:1' | '4:5' | '9:16' | '16:9' = '1:1';
...
function buildCreativePrompt(
  ...
  const userDirectionLine = options.userPrompt
    ? `Additional creative direction from the merchant: "${options.userPrompt}". Follow this direction as closely as possible.`
    : '';
...
LAYOUT:
- Composition and aspect ratio: ${DEFAULT_ASPECT_RATIO}.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    /*
    |--------------------------------------------------------------------------
    | Get Product (includes categoryId, subcategoryId, childCategoryId,
    | promptType, prompt, bannerColor that were saved at upload time)
    |--------------------------------------------------------------------------
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Resolve category / subcategory / child category names (best-effort)
    |--------------------------------------------------------------------------
    */
    ...
    if (product.categoryId) {
      try {
        ...
        console.error('CATEGORY LOOKUP ERROR:', lookupError);
      }
    }
    ...
    if (product.subcategoryId) {
      try {
        ...
        subcategoryName = (subcategoryRows[0] as any)?.name ?? null;
      } catch (lookupError) {
        console.error('SUBCATEGORY LOOKUP ERROR:', lookupError);
      }
    }

    // Child category is optional — only looked up if one was picked on upload.
    if (product.childCategoryId) {
      try {
        const childCategoryRows = await db
          .select()
          .from(childCategories)
          .where(eq(childCategories.id, product.childCategoryId))
          .limit(1);

        childCategoryName = (childCategoryRows[0] as any)?.name ?? null;
      } catch (lookupError) {
        console.error('CHILD CATEGORY LOOKUP ERROR:', lookupError);
      }
    }

    const userPrompt: string | null = product.prompt || null;
...
`.trim();
    ...
    */
    ...
    // Prefer the merchant's own category/subcategory/child-category
    // selection over the AI-guessed one, when the merchant picked one on
    // the upload screen.
